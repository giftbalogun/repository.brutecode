...
from brutecode import settings
from brutecode.log import log
from brutecode.session import Session
from brutecode.util import kodi_rpc, get_addon

from .constants import *

def get_brutecode_addons():
...
def check_updates(force=False):
    ...
    updates = []
    brutecode_addons = get_brutecode_addons()
    brutecode_installed = [x['addonid'] for x in kodi_rpc('Addons.GetAddons', {'installed': True, 'enabled': True})['addons'] if x['addonid'] in brutecode_addons]

    for addon_id in brutecode_installed:
        addon = get_addon(addon_id, install=False)
        if not addon:
            continue

        cur_version = addon.getAddonInfo('version')
        new_version = brutecode_addons[addon_id]['version']
