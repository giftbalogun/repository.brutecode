...
class Item(gui.Item):
    ...
    def play(self, **kwargs):
        ...
        if self.play_next:
            ...
            set_kodi_string('_brutecode_play_next', json.dumps(data))

        if self.play_skips:
            data = {'playing_file': self.path, 'skips': self.play_skips}
            set_kodi_string('_brutecode_play_skips', json.dumps(data))

        if self.callback:
            data = {'type': 'interval', 'playing_file': self.path, 'interval': 0, 'callback': None}
            data.update(self.callback)
            set_kodi_string('_brutecode_play_callback', json.dumps(data))
