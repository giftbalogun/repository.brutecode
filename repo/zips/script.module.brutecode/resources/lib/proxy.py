...
from brutecode import settings, gui, inputstream
from brutecode.log import log
from brutecode.constants import *
from brutecode.util import check_port, remove_file, get_kodi_string, set_kodi_string, fix_url, run_plugin
from brutecode.plugin import failed_playback
from brutecode.exceptions import Exit
from brutecode.session import RawSession
from brutecode.router import add_url_args
...
class RequestHandler(BaseHTTPRequestHandler):
    ...
    def _get_url(self):
        ...
        try:
            proxy_data = json.loads(get_kodi_string('_brutecode_quality'))
            if self._session.get('session_id') != proxy_data['session_id']:
                self._session = {}

            self._session.update(proxy_data)
            set_kodi_string('_brutecode_quality', '')
        except:
            pass
