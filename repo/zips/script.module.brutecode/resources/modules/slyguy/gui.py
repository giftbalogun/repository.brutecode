...
def refresh():
    set_kodi_string('brutecode_refresh', '1')
...
def exception(heading=None):
    ...
    include = [ADDON_ID,  os.path.join(COMMON_ADDON_ID, 'resources', 'modules', 'brutecode'), os.path.join(COMMON_ADDON_ID, 'resources', 'lib')]
    fline = True
...
class Item(object):
    ...
    def get_li(self):
        ...
        if self.path and (self.path.lower().startswith('http://') or self.path.lower().startswith('https://')):
            ...
            set_kodi_string('_brutecode_quality', json.dumps(proxy_data))
